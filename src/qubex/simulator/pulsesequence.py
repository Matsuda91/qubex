...
class RaisedCosFlatTop(Pulse):
    ...
    def _raised_cos(self):
        t = np.linspace(
            0, self.duration - SAMPLING_PERIOD, int(self.duration / SAMPLING_PERIOD)
        )
        shape = np.zeros(len(t))

        shape[t <= self.risetime] = 0.5 * (
            1 - np.cos(np.pi * t[t <= self.risetime] / self.risetime)
        )
        shape[t > self.duration - self.risetime] = 0.5 * (
            1
            - np.cos(
                np.pi
                * (t[(t > self.risetime) & (2 * self.risetime > t)] / self.risetime)
            )
        )
...
class Gaussian(Pulse):
    def __init__(self, label, amplitude, phase, duration):
        self.amplitude: float = amplitude
        self.phase: float = phase
        self.label: str = label
        self.duration: float = duration
    ...
    def _gaussian(self):
        ...
        shape = np.exp(
            -0.5 * ((t - self.duration / 2) ** 2) / ((sigma / self.n_sigma) ** 2)
        )
        return self.amplitude * shape * np.exp(1j * self.phase)
...
class Sequence:
    ...
    def _check_sampling_period(self):
        ...
        for _, combination in enumerate(
            itertools.combinations_with_replacement(frequencies, 2)
        ):
            diff.append(np.abs(combination[0] - combination[1]))
        if 1 / np.max(diff) < SAMPLING_PERIOD:
            raise ValueError(
                f"Sampling period is too large. The maximum frequency difference is {np.max(diff)}"
            )
        else:
            pass
    ...
    def add_pulse(self, pulses: list[Pulse], flush: str):
        for _, pulse in enumerate(pulses):
            self.sequences[pulse.label] = np.append(
                self.sequences[pulse.label], pulse.shape
            )
        self._padding(flush)
    ...
    def _padding(self, flush):
        ...
        if flush == "left":
            for kdx, label in enumerate(labels):
                self.sequences[label] = np.pad(
                    self.sequences[label], (0, int(max_length - lengthes[kdx]))
                )
        else:
            if flush != "right":
            ...
            for kdx, label in enumerate(labels):
                self.sequences[label] = np.pad(
                    self.sequences[label], (int(max_length - lengthes[kdx]), 0)
                )
        ...
        for _, transmon in enumerate(self.system.transmons):
            ...
            for _, coupling in enumerate(self.system.couplings):
                if "-" in coupling.label:
                    ...
                    channels[switch(coupling.label)] = self.system.transmons[
                        index0
                    ].frequency
                else:
                    pass
    ...
    def channels_information(self):
        info = {
            "labels": self.channels.keys(),
            "frequencies (GHz)": self.channels.values(),
        }
    ...
    def times(self) -> np.ndarray:
        ...
        return np.linspace(
            0, sequence_length - SAMPLING_PERIOD, int(sequence_length / SAMPLING_PERIOD)
        )
    ...
    def plot(self, yauto=True):
        ...
        for ldx, label in enumerate(self.sequences.keys()):
            ...
            plt.step(
                self.times, np.real(self.sequences[label]) / (2 * np.pi) * 1e3, zorder=1
            )
            plt.step(
                self.times, np.imag(self.sequences[label]) / (2 * np.pi) * 1e3, zorder=0
            )
